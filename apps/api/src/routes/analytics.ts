...
analyticsRouter.get("/national", async (req, res) => {
  try {
    ...
    const byState: Record<string, { costs: number[]; count: number; ac: number; suite: number; freshman: number }> = {};
    ...
    for (const d of dorms) {
      ...
      if (d.dormType === "FRESHMAN_ONLY" || (d.freshmanEligible && !d.upperclassEligible)) byState[st].freshman++;
    }

    const stateStats = Object.entries(byState).map(([state, data]) => ({
      state,
      avgCost: data.costs.length ? Math.round(data.costs.reduce((a, b) => a + b, 0) / data.costs.length) : 0,
    ...
    const publicCosts = dorms.filter((d) => d.college.schoolType === "PUBLIC" && d.yearlyCost).map((d) => d.yearlyCost!);
    const privateCosts = dorms.filter((d) => d.college.schoolType === "PRIVATE" && d.yearlyCost).map((d) => d.yearlyCost!);
    ...
    res.json({
      totals: {
        ...
        suiteBathrooms: Math.round((dorms.filter((d) => d.bathroomStyle === "SUITE").length / total) * 100),
        freshmanOnly: Math.round((dorms.filter((d) => d.dormType === "FRESHMAN_ONLY").length / total) * 100),
      },
      stateStats: stateStats.sort((a, b) => b.avgCost - a.avgCost),
      publicAvgCost: publicCosts.length ? Math.round(publicCosts.reduce((a, b) => a + b, 0) / publicCosts.length) : 0,
      privateAvgCost: privateCosts.length ? Math.round(privateCosts.reduce((a, b) => a + b, 0) / privateCosts.length) : 0,
      bathroomDist,
...
analyticsRouter.get("/quality", async (_req, res) => {
  try {
    ...
    res.json({ reports, missingCost, missingAmenities, lowConfidence });
  } catch (e) {
    res.status(500).json({ error: (e as Error).message });
  }
});
