/**
 * Backfill parent/child hierarchy from existing inventory evidence.
 * APPLY=1 to write.
 */
import { prisma } from "@dormscope/database";
import { enrichAllHierarchy } from "../../scraper/src/enrich/hierarchy.js";
...
async function main() {
  if (!APPLY) {
    console.log("=== DRY RUN — set APPLY=1 to link hierarchy ===");
  }
  ...
  console.log(JSON.stringify({ apply: APPLY, linked, suggested, results }, null, 2));
